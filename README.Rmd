...
```{r}
data_fuzzykmeans <- MicrogliaMorphologyR::data_fuzzykmeans
data_pca_fuzzykmeans <- cbind(pca_data[1:5], data_2xLPS, as.data.frame(data_fuzzykmeans$u), 
                              as.data.frame(data_fuzzykmeans$cluster) %>%
                                rename(Cluster=`data_fuzzykmeans$cluster`))
str(data_pca_fuzzykmeans)
clusterfeatures(pca_kmeans, start=11, end=37)

data_pca_fuzzykmeans %>% 
  filter(BrainRegion=="HC") %>%
  ggplot(aes(x=PC1, y=PC2, color=as.character(Cluster))) +
  facet_wrap(~Antibody*Sex*Treatment) +
  geom_point() +
  scale_color_viridis_d()

cp <- clusterpercentage(data_pca_fuzzykmeans, "Cluster", Antibody, Sex, Treatment, BrainRegion)
cp$Treatment <- factor(cp$Treatment, levels=c("PBS","2xLPS"))
