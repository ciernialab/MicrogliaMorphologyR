...
For demonstration purposes, we will use one of the datasets that comes packaged with MicrogliaMorphologyR. 'data_2xLPS' contains morphology data collected from female and male 8 week-old Cx3cr1-eGFP mice, which were given 2 i.p. injections of either PBS vehicle solution or 0.5mg/kg lipopolysaccharides (LPS), spaced 24 hours apart. Brains were collected 24 hours after the final injections, and brain sections were immunofluorescently stained and imaged for 3 different, commonly used microglia markers: Cx3cr1, P2ry12, and Iba1.

### TO DO
1. update data_2xLPS dataset
2. 
...
data_2xLPS %>% select(MouseID=="3"&BrainRegion=="HC") %>% View()

data_2xLPS <- data_2xLPS %>%
  filter(!(MouseID=="3" &BrainRegion=="HC"))
...
From using wss and silhouette methods to check the optimal numbers of clusters for our dataset, it appears that our data would be optimally clustered using k=4. There are many more cluster optimization methods that you can try out to explore your data (insert link). Next, we use the clusterfeatures function within MicrogliaMorphologyR to assess how a parameter of k = 4 influences how the clusters are defined by morphology features (and if they make sense according to what we know about microglia morphology). 
...
```{r}
# combine data
data_pca_fuzzykmeans <- cbind(pca_data[1:5], MicrogliaMorphologyR::data_2xLPS_fuzzykmeans)
str(data_pca_fuzzykmeans)

cp <- clusterpercentage(data_pca_fuzzykmeans, "Cluster", MouseID, Antibody, Sex, Treatment, BrainRegion)
cp$Treatment <- factor(cp$Treatment, levels=c("PBS","2xLPS"))
clusterpercentage_boxplots(cp %>% filter(BrainRegion=="STR"), Antibody, Sex, Treatment)

# explore data via boxplots
cp %>% filter(Antibody=="Iba1") %>%
...
```{r}
# transform data to be appropriate for anova
cp.stats <- cp %>% mutate(anova.input=ln(percentage/(100-percentage)))
cp.stats$Cluster <- factor(cp$Cluster)

# filter transformed dataset for downstream states
stats.input <- cp.stats %>% filter(BrainRegion=="FC", Antibody=="Iba1")

# run stats analysis for changes in cluster percentages, at the animal level
testing <- stats_cluster.animal(stats.input, "anova.input ~ Cluster*Treatment*Sex + (1|MouseID)",
                     "~Cluster*Treatment|Sex", "~Cluster*Treatment", "dunnet")
testing[[1]]
testing[[2]]
testing[[3]]
testing[[4]]
...
testing[[1]] %>% DT::datatable(., options=list(autoWidth=TRUE, scrollX=TRUE, scrollCollapse=TRUE))
testing[[2]] %>% DT::datatable(., options=list(autoWidth=TRUE, scrollX=TRUE, scrollCollapse=TRUE))
testing[[3]] %>% DT::datatable(., options=list(autoWidth=TRUE, scrollX=TRUE, scrollCollapse=TRUE))
```
